...
class AsanaContextProvider(ContextProvider):
    def __init__(self, project_id, fields=None, **kwargs):
        super().__init__()
    ...
    async def fetch_context_async(self, full_fetch: bool = False):
        ...
        try:
            ...
            if project:
                ...
                self.context_info = formatter.generate_project_markdown(project, tasks, self.fields)
    ...
    def fetch_context(self, full_fetch: bool = False):
        super().fetch_context(full_fetch)
...
class MarkdownFormatter:
    ...
    def format_date(date_str):
        if date_str:
            ...
            date = date.astimezone(pytz.timezone('America/Toronto'))
            return date.strftime('%B %-dth, %Y %-I:%M %p')
        return 'N/A'

    def generate_project_markdown(self, project, tasks, task_fields):
        markdown = ""
        markdown += self.heading1(f"Project: {project['name']}")
        markdown += self.format_field("Project ID", project['gid'])
        markdown += self.format_field("Created At", self.format_date(project['created_at']))
        markdown += self.format_field("Modified At", self.format_date(project['modified_at']))
        ...
        for task in tasks:
            ...
            for story in task['stories']:
                story_text = f"{self.format_date(story['created_at'])} by {story['created_by']['name']}: {story['text']}"
